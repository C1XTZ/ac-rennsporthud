function script.inputs(dt)
    ...
    if not settings.inputsShowWheel then position.inputs.elementsize.x = position.inputs.elementsize.x - (position.inputs.wheel.padding + position.inputs.wheel.imgsize + scale(1)) end

    ui.setCursor(vec2(0, app.padding))
    ui.childWindow('main', position.inputs.elementsize, function()
        ...
        if settings.inputsShowWheel then
            local wheelpos = vec2(scale(1), (position.inputs.pedalsize.y) / 2)
            local wheelimg
            if playerCar().isRacingCar then wheelimg = '.\\img\\RaceWheel.png' else wheelimg = '.\\img\\StreetWheel.png' end

            if settings.decor then wheelpos.y = wheelpos.y + position.inputs.decorheight end
            if not settings.inputsShowSteering then wheelpos.y = wheelpos.y - position.inputs.steeringbar.y / 2 end
            if not settings.inputsShowPedals then wheelpos.y = wheelpos.y - position.inputs.pedalsize.y / 2 end
        ...
        if settings.decor then
            ...
            ui.setCursor(vec2(math.round(horiOffset + position.inputs.pedalsize.x / 2 - position.inputs.decorimg.x / 2), math.round(position.inputs.decorheight / 2 - position.inputs.decorimg.y / 2)))
        ...
        if settings.inputsShowPedals then
            if settings.inputsShowFFB then
                ...
                ui.dwriteTextAligned('FORCE FEEDBACK', fontBig, ui.Alignment.Center, ui.Alignment.Center, vec2(ui.availableSpaceX(), position.inputs.pedalheight - scale(1)), false, txtcolor)
                ui.popDWriteFont()
            ...
            if settings.inputsShowClutch then
                ...
                ui.pushDWriteFont(app.font.bold)
                ui.dwriteTextAligned('CLUTCH', fontBig, ui.Alignment.Center, ui.Alignment.Center, vec2(ui.availableSpaceX(), position.inputs.pedalheight - scale(1)), false, txtcolor)
                ui.popDWriteFont()
                vertOffset = vertOffset + position.inputs.pedalheight
            end
            if settings.inputsShowBrake then
                ...
                ui.pushDWriteFont(app.font.bold)
                ui.dwriteTextAligned('BRAKE', fontBig, ui.Alignment.Center, ui.Alignment.Center, vec2(ui.availableSpaceX(), position.inputs.pedalheight - scale(1)), false, txtcolor)
                ui.popDWriteFont()
                vertOffset = vertOffset + position.inputs.pedalheight
            end
            if settings.inputsShowGas then
                ...
                ui.pushDWriteFont(app.font.bold)
                ui.dwriteTextAligned('THROTTLE', fontBig, ui.Alignment.Center, ui.Alignment.Center, vec2(ui.availableSpaceX(), position.inputs.pedalheight - scale(1)), false, txtcolor)
                ui.popDWriteFont()
                vertOffset = vertOffset + position.inputs.pedalheight
            end
        end
