  ...
  if (settings.deltaHidden and playerCar().estimatedLapTimeMs > 0) or not settings.deltaHidden then
    ...
    if settings.deltaShowTimer then
      ...
      ui.childWindow('DeltaTimer', vec2(position.delta.elementsize.x, position.delta.contentheight / 2 + vertOffset), false, app.flags, function()
        ...
        local deltaBestBox = vec2(ui.windowSize().x / 3, position.delta.contentheight / 2)
        local deltaBestFontSize = fitFontSize(deltaBestTxt, app.font.black, fontsize, deltaBestBox)
        ui.dwriteTextAligned(deltaBestTxt, deltaBestFontSize, 1, 0, deltaBestBox, false, deltaColor)
        ui.popDWriteFont()
      end)
    ...
    if settings.deltaShowPrediction then
      ...
      ui.childWindow('LapPrediction', vec2(position.delta.elementsize.x, position.delta.contentheight / 2 + vertOffset), false, app.flags, function()
        ...
        local predictionBox = vec2((ui.windowSize().x / 3) + scale(3), position.delta.contentheight / 2) - position.delta.txtpos.y
        local predictionFontSize = fitFontSize(predictionTxt, app.font.black, fontsize, predictionBox)
        ui.dwriteTextAligned(predictionTxt, predictionFontSize, 1, 0, predictionBox, false, txtcolor)
        ui.popDWriteFont()
      end)
      childOffset = childOffset + position.delta.contentheight / 2 + vertOffset
    end
    ...
    ui.setCursor(vec2(0, childOffset))
    ui.childWindow('EmptySpace', position.delta.elementsize, false, app.flags, function() end)
  end
end
