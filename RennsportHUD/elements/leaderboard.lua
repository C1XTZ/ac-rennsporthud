...
function updateLeaderboard()
  ...
  if not onlineVersionCheck then
    if session.type == ac.SessionType.Race then
      ...
      table.sort(lbTable, function(a, b) return a.pos < b.pos end)
    else
...
function script.leaderboard(dt)
  ...
  if lbTable then
    ...
    for i = 1, #lbTable do
      ...
      if not onlineVersionCheck then
        ...
        maxBestLength = math.max(maxBestLength, math.round(ui.measureDWriteText(lbTable[i].best, fontSizeSmall).x))
  ...
  local displayData = {
    lbShowPos = { width = math.max(position.leaderboard.pnl, maxPosLength), str = 'Pos.' },
    lbShowNum = { width = math.max(position.leaderboard.pnl, maxNumLength), str = 'Num' },
    ...
    lbShowLap = { width = math.max(position.leaderboard.lap, maxLapLength), str = 'Lap' },
    lbShowLast = { width = math.max(position.leaderboard.time, maxLastLength), str = 'Last' },
    lbShowBest = { width = math.max(position.leaderboard.time, maxBestLength), str = 'Best' },
